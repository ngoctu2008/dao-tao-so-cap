...
window.editHocVien = async function(maHv) {
    ...
    if(hv) {
        ...
        const warningContainer = document.getElementById('hv_KhoaThamGiaContainer');
        const reasonInput = document.getElementById('hv_LyDoHocLai');
...
async function saveHocVien(e) {
    ...
    // Check reason if returning student is being approved
    const warningContainer = document.getElementById('hv_KhoaThamGiaContainer');
    const reasonInput = document.getElementById('hv_LyDoHocLai');
    let newReason = '';

    if (warningContainer && !warningContainer.classList.contains('d-none')) {
        if (document.getElementById('hv_TrangThaiDuyet').value === 'Đã duyệt') {
            if (!reasonInput || reasonInput.value.trim() === '') {
                alert('Vui lòng nhập nguyên nhân xin học lại trước khi Duyệt học viên này.');
                return;
            }
            newReason = reasonInput.value.trim();
        }
    }
...
window.duyetHocVien = async function(maHv) {
    ...
    if(fetchResult && fetchResult.success && fetchResult.data) {
        hv = fetchResult.data.find(h => h.MaHV === maHv);
